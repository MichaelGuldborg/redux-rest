...
import Identifiable, {OptionalIdentifiable} from '../models/Identifyable';
...
export const crudRestActions = <T extends OptionalIdentifiable>(stateName: string, service: CrudServiceType<T>): CrudRestActionType<T> => {
    return {
        refresh: (): AppThunkAction<KnownCrudRestAction<T>> => {
